...
Construire un renderer PDF partagé et les modèles « PV Signature illustré » et « PV Signature épuré ». Reprendre toutes les capacités commerciales du modèle Dolibarr comparable, ajouter le design, les photos produits, l’étude conditionnelle avec les deux taux de dégradation, la signature en ligne native et le QR code. La courbe financière affiche des graduations lisibles et les repères horizontal/vertical du point d’amortissement. Toutes les données de la projection suivent les décimales totales Dolibarr via `price2num(..., 'MT')` et `price()`, sauf la colonne « Prix réseau » qui conserve `price2num(..., 'MU')`. Tester les pieds de page, documents multipages, accents et absence de photo.
